...
WEIGHTS = tf.constant([0.00134934769594975,	
           0.0548351842469636,	
           0.0923963767207384,	
           0.0952650592913482,	
           0.156380870740497,	
...
#WEIGHTS = np.array([0.1265395 , 0.97901945, 0.98779824, 0.98798795, 0.99297994,
...
def get_arguments():
    ...
    parser.add_argument('--weighted-loss', type=str2bool, nargs='?',const=True, default=WEIGHTED,
                        help='To weight loss by class ')
...
def create_loss(output, label, num_classes, ignore_label, weighted = False, head=WEIGHTS):
    ...
    if weighted:
        ...
        logits = tf.reshape(raw_pred, (-1, num_classes))
        ...
        labels = tf.reshape(tf.one_hot(label, depth=num_classes), (-1, num_classes))

        softmax = tf.nn.softmax(logits)

        cross_entropy = -tf.reduce_sum(tf.multiply(labels * tf.log(softmax + epsilon), head), axis=[1])

        reduced_loss = tf.reduce_mean(cross_entropy)
...
def main():
    ...
    loss_sub4 = create_loss(sub4_out, label_batch, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
    loss_sub24 = create_loss(sub24_out, label_batch, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
    loss_sub124 = create_loss(sub124_out, label_batch, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
    ...
    loss_sub4_val = create_loss(sub4_out_val, label_batch_val, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
    loss_sub24_val = create_loss(sub24_out_val, label_batch_val, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
    loss_sub124_val = create_loss(sub124_out_val, label_batch_val, args.num_classes, args.ignore_label, weighted=args.weighted_loss)
...
#        good_pred_re_n = tf.reshape(good_pred,[args.batch_size,-1])
#        good_label_re_n = tf.reshape(good_label,[args.batch_size,-1])
#        mavp, update_avp = tf.metrics.average_precision_at_k(good_label_re_n, good_pred_re_n, k=24)
#    
    ...
    for step in range(RANGE):
        ...
        try:
            if step % args.save_pred_every == 0:
                ...
                _ = sess.run(update_op_m, feed_dict=feed_dict)
                m = sess.run(mIoU, feed_dict=feed_dict)
                
                save(saver, sess, args.snapshot_dir, step)
                metris = np.array([step, loss_value,loss1, loss2, loss3, m])
                listy.append(metris) 
